...
source used by products that explicitly assemble and allow those tool names.

All Skills are instruction-only classpath resources: no scripts, process execution, network access, credentials, or
Tool grants. The Git/GitHub Skills declare `execution_run` as the required tool name; products still need a frozen
`execution_run` Tool, Workspace/network authorization, Policy, and Approval.
