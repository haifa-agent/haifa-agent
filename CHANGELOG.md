...
- Coding Agent and CLI distribution defaults retire the built-in `git-delivery` Skill. Git delivery workflows and control-plane boundaries are handled directly by the model through the system `git`/`gh` executables without guessing auxiliary resource paths. Shipped distribution template `distribution/haifa-coding.yaml` and default configurations removed `git-delivery` from allowed skills. Existing custom configurations explicitly naming `git-delivery` under `skills.allowed` must remove it to avoid startup failure during allowed Skill availability validation.
