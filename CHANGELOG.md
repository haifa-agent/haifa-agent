...
- Coding Agent and CLI distribution defaults retire the built-in `git-delivery` Skill. Git delivery workflows and control-plane boundaries are now handled directly through the shared `git` and `github` Skills without guessing auxiliary resource paths. Shipped distribution template `distribution/haifa-coding.yaml` and default configurations remove `git-delivery` from allowed skills (`skills.allowed: [task-planning, result-verification, git, github]`). Existing custom configurations explicitly naming `git-delivery` under `skills.allowed` must remove it to avoid startup failure during allowed Skill availability validation.
