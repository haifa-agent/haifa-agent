...
- 基础 SQLite Store 保持 Runtime-only，不依赖 Orchestration Core，也不创建 `workflow_*` 表；可选的
  `haifa-agent-store-sqlite-orchestration` 必须显式装配，并在当前 Runtime V11 之后使用 V12/V13 Migration。
