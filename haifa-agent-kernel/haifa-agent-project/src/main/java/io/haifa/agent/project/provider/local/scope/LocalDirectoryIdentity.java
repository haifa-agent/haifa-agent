 ...
 */
public final class LocalDirectoryIdentity {
    private static final String NAMESPACE = "io.haifa.agent.project.local-authorized-directory/v1";

    private final String fingerprint;
    private final String digest;

    private LocalDirectoryIdentity(String fingerprint, String digest) {
        this.fingerprint = fingerprint;
        this.digest = digest;
    }

    public static LocalDirectoryIdentity resolve(Path realDirectory) {
        ...
        try {
            String fingerprint = LocalWorkspaceLocationStore.fingerprintFor(realDirectory);
            String platform =
                    System.getProperty("os.name", "").toLowerCase(Locale.ROOT).contains("win") ? "windows" : "posix";
            return new LocalDirectoryIdentity(fingerprint, sha256(NAMESPACE + "\0" + platform + "\0" + fingerprint));
