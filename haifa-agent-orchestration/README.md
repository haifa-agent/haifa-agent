...
`haifa-agent-store-sqlite-orchestration` integration implements them from SQLite V9 while the base Runtime Store stays
at V8 without Workflow tables.
M5 adds frozen static subgraphs with explicit State mapping, child Workflow Runs, cancellation/timeout propagation,
and a SQLite V10 parent/child relation.
