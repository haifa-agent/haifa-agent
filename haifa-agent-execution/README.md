...
依赖方向：`execution-core -> execution-api + sandbox-api + project`，
`sandbox-host -> sandbox-api + project`，
`sandbox-local-native -> sandbox-api + project`。只有 Host 与 Local Native 两个精确的具体
Provider 包允许直接使用 Java 进程 API；Project Tool、Runtime、CLI 和 MCP 均通过
`ExecutionBroker`。
