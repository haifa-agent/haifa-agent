...
record CliConfiguration(
        Model model, Set<String> enabledTools, ApprovalMode approval, Duration timeout, int maxIterations, long maxToolCalls) {
    private static final Set<String> DEFAULT_TOOLS = Set.of(
            "file.list", "file.stat", "file.read", "file.search", "file.create", "file.write", "file.delete", "file.move");

    CliConfiguration {
        model = Objects.requireNonNull(model, "model must not be null");
        enabledTools = Set.copyOf(new LinkedHashSet<>(Objects.requireNonNull(enabledTools, "enabledTools must not be null")));
        ...
        if (maxIterations < 1 || maxToolCalls < 1) throw new IllegalArgumentException("runtime limits must be positive");
    }

    static CliConfiguration defaults() {
        return new CliConfiguration(
                new Model("deepseek", "deepseek-v4-pro", URI.create("https://api.deepseek.com"), "env://DEEPSEEK_API_KEY"),
                DEFAULT_TOOLS,
                ApprovalMode.ASK,
