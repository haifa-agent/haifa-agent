...
final class ProjectApplicationMappers {
    ...
    private static final String WORKSPACE_ACCESS_MAPPER =
            "/io/haifa/agent/application/project/persistence/CodingWorkspaceAccessMapper.xml";

    private ProjectApplicationMappers() {}

    static List<MapperXml> all() {
        return List.of(
                read(PRODUCT_SESSION_MAPPER),
                read(CODING_SESSION_MAPPER),
                read(WORKSPACE_REGISTRY_MAPPER),
                read(WORKSPACE_ACCESS_MAPPER));
    }
