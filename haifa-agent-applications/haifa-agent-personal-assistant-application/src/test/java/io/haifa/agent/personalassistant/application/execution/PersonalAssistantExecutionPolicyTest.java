...
class PersonalAssistantExecutionPolicyTest {
    private final PersonalAssistantExecutionPolicy policy = new PersonalAssistantExecutionPolicy();

    @Test
    void allowsOnlyRuntimeToolExecutionWithExactCorrelation() {
        ...
        assertThatThrownBy(() -> policy.authorize(
                        request(ExecutionOrigin.PRODUCT_USER_COMMAND, Optional.empty(), Set.of("execution.run"))))
                .isInstanceOf(ExecutionRejectedException.class)
                .hasMessageContaining("does not authorize");
        assertThatThrownBy(() -> policy.authorize(
                        request(ExecutionOrigin.PRODUCT_INTERNAL, Optional.empty(), Set.of("execution.run"))))
                .isInstanceOf(ExecutionRejectedException.class);
    }

    private static ExecutionRequest request(
            ExecutionOrigin origin, Optional<ToolCallId> toolCallId, Set<String> capabilities) {
        ...
        return new ExecutionRequest(
                ...
                new ExecutionCommand(ExecutionCommandMode.DIRECT, List.of("tool", "--version")),
