          ...
          <div className="mission-content">
            ...
            {!creatingMission && (selected ? (
              <article className="mission-detail">
                ...
                {missionInteraction && <section className="mission-interaction"><h4>{missionInteraction.approvalPresentation?.title ?? missionInteraction.title}</h4>{missionInteraction.approvalPresentation ? <><p>{missionInteraction.approvalPresentation.purpose}</p><pre className="approval-summary">{missionInteraction.approvalPresentation.content}</pre>{missionInteraction.approvalPresentation.environment.length > 0 && <ul className="approval-facts">{missionInteraction.approvalPresentation.environment.map((fact) => <li key={fact.label}><span>{fact.label}</span><strong>{fact.value}</strong></li>)}</ul>}</> : <p>{missionInteraction.safePrompt}</p>}{missionInteraction.inputType !== "NONE" && <textarea value={missionInteractionText} maxLength={missionInteraction.maximumCharacters} onChange={(event) => setMissionInteractionText(event.target.value)} rows={3} /> }<div>{missionInteraction.allowedActions.map((action) => <button key={action} type="button" className="button" disabled={busy} onClick={() => respondToMissionInteraction(action)}>{action}</button>)}</div></section>}
