...
用户凭据存储在操作系统提供的安全凭据管理器（`WindowsLocalModelAuthStore`）中，历史明文 `auth.json` 已彻底废弃，
不再读取或迁移；重新认证成功后提供安全的手动删除旧文件提示。Secret 不得进入日志、异常、公共 View、Runtime SQLite、JSONL 或 Workspace。
...
`EXCHANGING`，换取并校验 Token 后由 Coordinator 进入 `STORING`，只有系统凭据管理器持久化成功才进入
