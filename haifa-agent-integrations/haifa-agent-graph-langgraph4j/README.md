...
Pure Java M2 Integration Adapter for `org.bsc.langgraph4j:langgraph4j-core:1.8.24`.
...
- Provider `MemorySaver` is only an in-process continuation detail. It is not a Haifa durable store or a production
  recovery claim.
- Subgraphs, dynamic fan-out, `ANY_OF`, Provider-native model/tool actions, Spring AI, Studio, and Provider database
  savers are not supported.
