...
class SiliconFlowLiveIT {
    ...
    private static final String TOOL_RESULT_MARKER = "SILICONFLOW_TOOL_OK_8421";
    ...
    void streamsContentAndPublishesOneFinalUsageWhenExplicitlyEnabled() {
                ...
                .invokeStreaming(
                        request(
                                List.of(ModelMessage.text(
                                        ...
                                        "Reply with one short sentence containing SILICONFLOW_STREAM_OK.")),
                                List.of()),
                        event -> {
                            events.add(event);
        ...
        assertThat(response.content()).contains("SILICONFLOW_STREAM_OK");
    ...
    private static String requireLiveExecution() {
        Assumptions.assumeTrue("true".equalsIgnoreCase(System.getenv("HAIFA_SILICONFLOW_LIVE_TEST")));
        String apiKey = System.getenv("SILICONFLOW_API_KEY");
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException("SILICONFLOW_API_KEY is required for explicit live execution");
        }
        return apiKey;
    }
    ...
    private static ModelProviderDefinition provider() {
        ...
        return new ModelProviderDefinition(
                ...
                List.of(model),
                OpenAiCompatibleDialects.configuredOptions(OpenAiCompatibleDialects.SILICONFLOW, endpoint),
                Map.of());
    }
    ...
    private static AgentChatRequest request(List<ModelMessage> messages, List<ModelToolSpecification> tools) {
        return new AgentChatRequest(
                new ModelCallId("siliconflow-live-call"),
                new AgentRunId("siliconflow-live-run"),
                1,
                1,
                snapshot(),
                messages,
