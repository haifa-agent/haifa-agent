...
class CredentialPlanTest(unittest.TestCase):
    ...
    def distribution(self, directory: str) -> Path:
        ...
        (root / "haifa-coding.yaml").write_text(
            ...
            + "execution:" + chr(10)
            + "  provider: host-guarded" + chr(10)
            + "  maxTimeoutMillis: 1800000" + chr(10),
            encoding="utf-8",
        )
        launcher = root / "haifa-coding.cmd"
        launcher.write_text("@echo off" + chr(10), encoding="utf-8")
    ...
    def test_an_unchanged_credential_source_keeps_the_configured_reference(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            text = MODULE.agent_configuration(configured, 600).read_text(encoding="utf-8")

        self.assertIn("credentialRef: model-auth://deepseek/default", text)
        self.assertNotIn("credentialRef: env://DEEPSEEK_API_KEY", text)

    def test_one_command_may_not_hold_more_than_a_quarter_of_the_case_budget(self):
        self.assertEqual(135_000, MODULE.execution_timeout_millis(540))
        self.assertEqual(405_000, MODULE.execution_timeout_millis(1620))
        # A tiny budget still leaves a command a usable minute.
        self.assertEqual(60_000, MODULE.execution_timeout_millis(60))

    def test_the_command_timeout_of_the_distribution_is_replaced_per_case(self):
        with tempfile.TemporaryDirectory() as directory:
            launcher = self.distribution(directory)
            self.stored.add("haifa:model-auth:deepseek/default")
            configured = settings(agent=str(launcher), output_dir=Path(directory) / "out")

            short = MODULE.agent_configuration(configured, 540)
            long = MODULE.agent_configuration(configured, 1620)
            again = MODULE.agent_configuration(configured, 540)

            self.assertIn("maxTimeoutMillis: 135000", short.read_text(encoding="utf-8"))
            self.assertIn("maxTimeoutMillis: 405000", long.read_text(encoding="utf-8"))
            self.assertNotIn("1800000", short.read_text(encoding="utf-8"))
            self.assertEqual(1, short.read_text(encoding="utf-8").count("maxTimeoutMillis:"))
        # Each budget is written once, and a case never reads another case's file.
        self.assertEqual(short, again)
        self.assertNotEqual(short, long)

    def test_a_launcher_without_a_distribution_configuration_needs_no_rewrite(self):
        self.assertIsNone(MODULE.agent_configuration(settings(agent="haifa-coding"), 540))
