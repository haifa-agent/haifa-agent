...
CRED_TYPE_GENERIC = 1
# One shell command may hold at most this fraction of a case budget, and never less than a minute.
EXECUTION_TIMEOUT_DIVISOR = 4
MINIMUM_EXECUTION_TIMEOUT_MILLIS = 60_000
...
class Settings:
    ...
    # Distribution configuration rewritten per case budget, keyed by that budget in seconds.
    agent_configs: dict[int, Path] = field(default_factory=dict)
...
def execution_timeout_millis(budget_seconds: int) -> int:
    """Cap one shell command at a quarter of the case budget.

    The distribution lets a command run until the product maximum, which the run deadline then
    reduces to whatever is left: a command that never returns consumes the whole case and the agent
    never learns that it hung. A quarter leaves room to notice the timeout and try something else.
    """
    return max(MINIMUM_EXECUTION_TIMEOUT_MILLIS, budget_seconds * 1000 // EXECUTION_TIMEOUT_DIVISOR)


def write_agent_configuration(settings: "Settings", plan: CredentialPlan, budget_seconds: int) -> Path | None:
    """Rewrite the distribution configuration for this run: credential source and command timeout.

    The copy carries a credential reference such as ``env://NAME``, never a credential value.
    """
    configuration = distribution_configuration(settings.agent)
    if configuration is None:
        return None
    timeout = execution_timeout_millis(budget_seconds)
    rewrite_credential = bool(plan.provider) and bool(plan.reference) and credential_reference(settings.agent, plan.provider) != plan.reference
    source = configuration.read_text(encoding="utf-8").splitlines(keepends=True)
    # A distribution that leaves the command timeout at its default still gets the cap.
    declared = any(line.strip().startswith("maxTimeoutMillis:") for line in source)
    lines, current, capped = [], None, False
    for line in source:
        stripped = line.strip()
        indent = line[: len(line) - len(line.lstrip())]
        if stripped.startswith("- id:"):
            current = stripped.split(":", 1)[1].strip()
        elif rewrite_credential and stripped.startswith("credentialRef:") and current == plan.provider:
            line = f"{indent}credentialRef: {plan.reference}\n"
        elif stripped.startswith("maxTimeoutMillis:"):
            line = f"{indent}maxTimeoutMillis: {timeout}\n"
            capped = True
        lines.append(line)
        if stripped == "execution:" and not declared:
            lines.append(f"  maxTimeoutMillis: {timeout}\n")
            capped = True
    if not capped:
        lines.append(f"execution:\n  maxTimeoutMillis: {timeout}\n")
    settings.output_dir.mkdir(parents=True, exist_ok=True)
    target = settings.output_dir / f"agent-configuration-{timeout // 1000}s.yaml"
    target.write_text("".join(lines), encoding="utf-8")
    return target


def agent_configuration(settings: "Settings", budget_seconds: int) -> Path | None:
    """Return the configuration this case runs with, writing it once per distinct budget."""
    cached = settings.agent_configs.get(budget_seconds)
    if cached is not None:
        return cached
    written = write_agent_configuration(settings, plan_credential(settings), budget_seconds)
    if written is not None:
        settings.agent_configs[budget_seconds] = written
    return written
...
def evaluate(settings: Settings, checkout: Path, case_ids: list[str], provenance: AssetProvenance) -> int:
    ...
    if not settings.rehearse and settings.agent:
        ...
        plan = plan_credential(settings)
        if plan.reference:
            say(f"  credential: {plan.provider} reads {plan.reference}")
    say()
