...
- `fixtures/autonomous-delivery/`：十七个合成 Coding Case 的 Prompt、初始 Workspace、外置
  ...
  混合比较。
- `fixtures/coding-runtime-reliability/replay-v1.json`：从聚合失败形态提炼的九个合成 Replay Contract，
  覆盖退出码、Workspace 恢复、Model/Context、复数 ChangeSet 和 Push outcome unknown；删除 Git/GH
  分类器后不再断言 Operation Hint、Git 读取风险或复合命令分类。不包含真实 Prompt、命令输出、Provider
  响应、凭据、仓库数据或主机路径。
